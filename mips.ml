...
let sp = "$sp"
let ra = "$ra"
let fp = "$fp"   (* Ajout du Frame Pointer *)
let zero = "$zero" (* Ajout du registre Zéro *)

open Printf
(* Le reste du fichier est correct, mais assurez-vous que 'sub' est bien utilisé avec des registres *)
...
(* Helpers pour liste, dword, asciiz... inchangés *)
let rec ilist = function
    | []     -> ""
    | [i]    -> sprintf "%d" i
    | i :: l -> sprintf "%d, %s" i (ilist l)
let dword l  = S(sprintf "  .word %s" (ilist l))
let asciiz s = S(sprintf "  .asciiz %S" s) (* Utiliser %S pour quoter la string correctement *)
