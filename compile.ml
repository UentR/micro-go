...
let compile_struct_printers () =
  StringMap.fold (fun sname layout code ->
    ...
    (* On génère le code pour afficher chaque champ *)
    let rec print_fields = function
      | [] -> nop
      | (_, (off, t)) :: rest ->
           let load = lw a0 off s0 in
           let print_val = match t with
             | TString -> li v0 4 @@ syscall
             | TStruct _ -> li v0 1 @@ syscall 
             | _ -> li v0 1 @@ syscall
           in
           let sep = if rest <> [] then la a0 str_space @@ li v0 4 @@ syscall else nop in
           
           load @@ print_val @@ sep @@ print_fields rest
    in
...
and tr_expr env e = match e.edesc with
  ...
  | Call (f, args) ->
      ...
      let push_args = List.fold_left (fun acc arg -> 
  ...
  | Binop(bop, e1, e2) ->
      let op = match bop with
        ...
        | And -> and_ | Or -> or_ 
      in
      let c2 = tr_expr env e2 in
      let c1 = tr_expr env e1 in
      
      c2 @@ push t0 @@ c1 @@ pop t1 @@ op t0 t0 t1
  ...
  | Print(exps) -> 
      ...
      let compile_print_single e = 
        ...
        | Call(f, args) when List.length (get_func_sig f.id).rets > 1 ->
            (* Cas spécial : print d'une fonction à retour multiple *)
            let sig_f = get_func_sig f.id in
            let rets = sig_f.rets in
            let n_rets = List.length rets in
            let n_args = List.length args in
            
            (* Allocation place résultats *)
            let alloc_res = 
            ...
            (* Passage pointeurs résultats *)
            let push_ptrs = 
              let rec loop i = 
                 if i >= n_rets then nop else 
                 let off = 4 * n_args + 4 * (n_rets - 1 - i) + (i * 4) in
                 addi t0 sp off @@ push t0 @@ loop (i + 1) 
              in loop 0 
            in
            ...
            let print_code = 
              let rec loop i = 
                 ...
                 let load = lw t0 off sp in
                 let print_val = match t with 
                    | TString -> move a0 t0 @@ li v0 4 @@ syscall 
                    | TStruct s -> move a0 t0 @@ jal ("print_struct_" ^ s)
                    | _ -> move a0 t0 @@ li v0 1 @@ syscall
                 in
                 let space = if i < n_rets - 1 then print_space else nop in
                 load @@ print_val @@ space @@ loop (i+1) 
              in loop 0 
            in
        ...
        | _ ->
            let code_expr = tr_expr env e in
            let type_list = get_expr_type env e in
            let print_instruction = match type_list with
                | [TString] -> move a0 t0 @@ li v0 4 @@ syscall
                | [TStruct s] -> move a0 t0 @@ jal ("print_struct_" ^ s)
                | _ -> move a0 t0 @@ li v0 1 @@ syscall
            in
            code_expr @@ print_instruction
      in
...
and tr_instr env i = match i.idesc with 
  ...
  | Set (lvl, el) ->
      (match el with
       | [{edesc = Call(f, args); _}] when List.length (get_func_sig f.id).rets > 1 ->
           (* Affectation multiple via appel de fonction *)
       ...
       | _ -> 
         ...
         let pop_assign = List.fold_right (fun lv code -> 
  ...
  | Vars (ids, t_opt, seq_body) ->
      let types = match t_opt with 
        ...
        | None -> 
            ...
            | {idesc=Set(_, el);_}::_ -> 
                (match el with 
                 | [{edesc=Call(f,_);_}] -> (get_func_sig f.id).rets 
                 | _ -> List.flatten (List.map (get_expr_type env) el)) 
            | _ -> List.map (fun _ -> TInt) ids 
      in
      
      let new_env, alloc = List.fold_left2 (fun (e, code) id t -> 
          let off = e.next_local in 
          let new_vars = StringMap.add id.id (off, t) e.vars in
          let next = off - 4 in
          ({e with vars=new_vars; next_local=next}, code @@ li t0 0 @@ push t0)
      ) (env, nop) ids types in
  ...
  | If(c, s1, s2) ->
      let l1 = new_label() in 
      let l2 = new_label() in
      let code_cond = tr_expr env c in
      code_cond @@ bnez t0 l1 @@ tr_seq env s2 @@ j l2 @@ label l1 @@ tr_seq env s1 @@ label l2
  ...
  | Return el ->
      if env.ret_ptr_offsets <> [] then 
        ...
        | _ -> 
            let store_returns = List.fold_left2 (fun acc expr off -> 
                acc @@ tr_expr env expr @@ lw t1 off fp @@ sw t0 0 t1
            ) nop el env.ret_ptr_offsets in
            store_returns @@ j env.exit_label)
      else 
        (match el with 
         | [] -> j env.exit_label 
...
let tr_fun df =
  ...
  let env_p, _ = List.fold_left (fun (e, i) (id, t) -> 
      let off = 8 + 4*(tot-1-i) in 
      ({e with vars=StringMap.add id.id (off, t) e.vars}, i+1)
  ) ({empty_env with exit_label=exit_lbl}, 0) df.params in
  
  let env = if n_imp > 0 then 
      let rec loop i acc = if i>=n_imp then (List.rev acc) else loop (i+1) ((8 + 4*(n_imp-1-i))::acc) in 
      {env_p with ret_ptr_offsets=loop 0 []} 
  else env_p in
...
let tr_prog decls =
  List.iter (function 
    ...
    | Fun f -> func_table := StringMap.add f.fname.id {nargs=List.length f.params; rets=f.return} !func_table
  ) decls;
  ...
  let printers = compile_struct_printers () in 
