...
let check loc expected actual =
  if expected <> actual then
    error loc ("type mismatch: expected " ^ (string_of_typ expected) ^ " but got " ^ (string_of_typ actual))
...
let check_wf loc env = function
  | TStruct s when not (StringMap.mem s env.structs) -> error loc ("structure not declared: " ^ s)
  | _ -> ()
...
let rec type_expr env e =
  ...
  | Nil -> error e.eloc "nil cannot be used without context"
  | Var id ->
      (try StringMap.find id.id env.vars 
       with Not_found -> error id.loc ("variable not found: " ^ id.id))
  | New s ->
      if not (StringMap.mem s env.structs) then error e.eloc ("unknown struct type: " ^ s);
      TStruct s
  | Dot (e, id) ->
      (match type_expr env e with
       | TStruct s ->
           let fields = (StringMap.find s env.structs).fields in
           (try StringMap.find id.id fields 
            with Not_found -> error id.loc ("field " ^ id.id ^ " does not exist in struct " ^ s))
       | _ -> error e.eloc "dot access requires a struct pointer")
  | Call (f, args) ->
      let info = try StringMap.find f.id env.funcs 
                 with Not_found -> error f.loc ("undefined function " ^ f.id) in
      check_args env f.loc info.params args;
      (match info.rets with 
       | [t] -> t 
       | _ -> error e.eloc "function call returns multiple values, cannot be used as expression")
  | Unop (op, e1) ->
      let t = type_expr env e1 in
      (match op, t with 
       | Opp, TInt -> TInt 
       | Not, TBool -> TBool 
       | _ -> error e.eloc "invalid unary operation for this type")
  | Binop ((Eq | Neq), e1, e2) ->
      let t1, t2 = try type_expr env e1, type_expr env e2 with _ -> TInt, TInt in
      if e1.edesc = Nil && e2.edesc = Nil then error e.eloc "cannot compare nil with nil";
      if e1.edesc <> Nil && e2.edesc <> Nil && t1 <> t2 then error e.eloc "types do not match in comparison";
      TBool
  | Binop (op, e1, e2) ->
      ...
      t_out
  | Print _ -> error e.eloc "fmt.Print is a statement, not an expression"

and check_args env loc expected args =
  if List.length expected <> List.length args then error loc "wrong number of arguments";
  List.iter2 (fun t arg ->
    if arg.edesc = Nil then 
        (match t with TStruct _ -> () | _ -> error arg.eloc "nil is only valid for pointers")
    else check arg.eloc t (type_expr env arg)
  ) expected args

(* PASSE 2 : INSTRUCTIONS *)

let check_lvalue e = match e.edesc with 
  | Var _ | Dot _ -> () 
  | _ -> error e.eloc "left-hand side must be a variable or field"

let get_rhs_types env exprs =
  match exprs with
  | [{edesc = Call(f, args); _}] ->
      let info = try StringMap.find f.id env.funcs with Not_found -> error f.loc "function not found" in
      check_args env f.loc info.params args;
      info.rets
  | _ ->
...
let rec check_instr env instr =
      ...
      (match e.edesc with
       | Print args ->
           List.iter (fun arg ->
             ...
             | Nil -> error arg.eloc "cannot print nil directly"
             | _ -> ignore (type_expr env arg)) args
       | _ -> ignore (type_expr env e))
  | Set (lvl, el) ->
      let types = get_rhs_types env el in
      if List.length lvl <> List.length types then error instr.iloc "assignment mismatch: count of variables and values differs";
  ...
  | Vars (ids, t_opt, seq) ->
      ...
      if List.length distinct <> List.length ids then error instr.iloc "duplicate variable names in declaration";
      let new_vars = match t_opt, seq with
        ...
        | None, {idesc=Set(_, el);_}::_ ->
            let types = get_rhs_types env el in
            if List.length ids <> List.length types then error instr.iloc "variables and values count mismatch in init";
            List.fold_left2 (fun acc id t -> if id.id="_" then acc else StringMap.add id.id t acc) env.vars ids types
        | _ -> error instr.iloc "invalid variable declaration"
      in
      check_seq { env with vars = new_vars } seq
  | Inc e | Dec e -> 
      check_lvalue e; 
      check e.eloc TInt (type_expr env e)
  | If (c, b1, b2) -> 
      check c.eloc TBool (type_expr env c); 
      check_seq env b1; 
      check_seq env b2
  | For (c, b) -> 
      check c.eloc TBool (type_expr env c); 
  ...
  | Return el ->
      let types = get_rhs_types env el in
      if List.length types <> List.length env.ret_ty then error instr.iloc "incorrect number of return values";
...
let prog (_, decls) =
  let structs = List.fold_left (fun acc -> function
    | Struct s -> 
        if StringMap.mem s.sname.id acc then error s.sname.loc ("struct " ^ s.sname.id ^ " is already defined");
  ...
  let env = List.fold_left (fun e -> function
    | Fun f ->
        if StringMap.mem f.fname.id e.funcs then error f.fname.loc ("function " ^ f.fname.id ^ " already exists");
    ...
    | Struct s ->
        let fields = List.fold_left (fun facc (id, t) ->
          check_wf id.loc e t;
          if StringMap.mem id.id facc then error id.loc ("duplicate field " ^ id.id);
  ...
  List.iter (function
    ...
    | Fun f ->
        ...
        if f.return <> [] && not (returns_all f.body) then error f.fname.loc "missing return statement at end of function body"
  ) decls;
  if not (StringMap.mem "main" env.funcs) then 
    error (Lexing.dummy_pos, Lexing.dummy_pos) "missing main function";
  decls